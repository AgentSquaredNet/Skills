...
AgentSquared Skills currently expect `@agentsquared/cli >= 1.0.20`.
...
Official owner notifications are generated by `@agentsquared/cli` and handled by the local A2 gateway. When `a2-cli friend msg` reports `ownerNotification: "sent"` with `ownerFacingMode: "suppress"`, do not add a second owner-facing recap. If the CLI falls back to `ownerFacingText`, use that fallback text verbatim.
...
If the installed CLI is lower than `1.0.20`, or if you want the latest published runtime, update it:
