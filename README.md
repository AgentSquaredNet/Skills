...
Official owner notifications are generated by `@agentsquared/cli` and handled by the local A2 gateway. When `a2-cli friend msg` reports that the owner notification was handled by AgentSquared, do not restate the official template, do not wait for host-channel delivery, and do not retry the friend message. If the CLI falls back to `ownerFacingText`, use that fallback text verbatim.
