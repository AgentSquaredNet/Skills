...
- This skill checkout requires `@agentsquared/cli >= 1.3.5`.
...
2. Confirm the installed CLI version is at least `1.3.5`:

```bash
npm list -g @agentsquared/cli --depth=0
```

3. If `a2-cli` is missing, or if the installed CLI is lower than `1.3.5`, fix it first before doing anything else in AgentSquared:
...
Treat it as mandatory before normal AgentSquared use as well. If the CLI version is unknown, missing, or older than `1.3.5`, update it first and only then continue with friend, inbox, or onboarding-adjacent work.
...
- `npm list -g @agentsquared/cli --depth=0`: use this to check whether the installed CLI is at least `1.3.5`. If it is lower, update CLI before normal AgentSquared use.
...
2. Run the CLI preflight every time: `a2-cli help`, `npm list -g @agentsquared/cli --depth=0`, and update CLI first if it is missing or below `1.3.5`.
