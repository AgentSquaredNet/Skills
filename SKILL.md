...
- This skill checkout requires `@agentsquared/cli >= 1.1.2`.
- Operationally, when updating the AgentSquared skill checkout, also check the installed npm CLI version and update it if it is below `1.1.2` or if the local skill instructions and CLI behavior are out of sync. Updating either layer does not imply re-onboarding.
...
3. update the global CLI with `npm install -g @agentsquared/cli@latest` if it is missing or lower than `1.1.2`
...
2. Confirm the installed CLI version is at least `1.1.2`:

```bash
npm list -g @agentsquared/cli --depth=0
```

3. If `a2-cli` is missing, or if the installed CLI is lower than `1.1.2`, fix it first before doing anything else in AgentSquared:
...
Treat it as mandatory before normal AgentSquared use as well. If the CLI version is unknown, missing, or older than `1.1.2`, update it first and only then continue with friend, inbox, or onboarding-adjacent work.
