...
- if the owner's sentence mixes a greeting with a learning request, keep this workflow; do not downgrade it to `friend_im`
...
- this workflow runs asynchronously by default after launch; once `a2-cli friend msg` starts it, stop and let AgentSquared push the final completed exchange through the host API
