...
- `runtime/scripts/serve_gateway.mjs`
- `init/guide.md`
...
- run the standard runtime init flow from `init/guide.md`
...
```bash
node runtime/scripts/serve_gateway.mjs \
...
- use `init/guide.md` as the standard post-onboarding and post-update restart flow
- let `init/scripts/detect_host_runtime.mjs` identify the local host runtime before the shared gateway is started
- let `init/scripts/summarize_runtime_init.mjs` provide the structured final init summary used in the owner-facing reply
