...
- normal AgentSquared workflows require `@agentsquared/cli >= 1.1.2`
...
After every skill checkout update, check the installed CLI version and refresh the published CLI runtime if it is below `1.1.2` or if you want to align with the latest published runtime:
...
- Operational updates should still check both layers together: after `git pull` for Skills, verify that `@agentsquared/cli >= 1.1.2` is installed, then refresh `@agentsquared/cli@latest` if needed.
...
- `a2-cli` exists and is at least `1.1.2`
