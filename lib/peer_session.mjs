...
import { dialProtocol, readJsonMessage, waitForPublishedTransport, writeLine } from './libp2p_a2a.mjs'
...
export async function openDirectPeerSession({
  ...
  const relayAttempt = await createConnectTicketWithRecovery({
    apiBase,
    agentId,
    bundle,
    node,
...
async function exchangeOverTransport({
  ...
  try {
    stream = await dialProtocol(node, transport, { requireDirect: false })
