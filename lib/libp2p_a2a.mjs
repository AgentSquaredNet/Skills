...
function isRelayCircuitAddr(value) {
...
export async function createNode({
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      const failedRelayAddrs = failedListenAddrs.filter(isRelayCircuitAddr)
      if (failedRelayAddrs.length === 0) {
        throw error
      }
      const nextListenAddrs = activeListenAddrs.filter((value) => !failedRelayAddrs.includes(value))
      const removedAny = nextListenAddrs.length < activeListenAddrs.length
      const stillHasDirect = nextListenAddrs.some((value) => !isRelayCircuitAddr(value))
      if (!removedAny || !stillHasDirect) {
        throw error
      }
      activeListenAddrs = nextListenAddrs
    }
  }
...
export async function readJsonMessage(stream) {
  let buffer = ''
  for await (const chunk of stream) {
    buffer += Buffer.from(chunk.subarray ? chunk.subarray(0) : chunk.slice()).toString('utf8')
    ...
    while (true) {
      ...
      const candidate = buffer.slice(0, newlineIndex).trim()
