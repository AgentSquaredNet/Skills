...
export function buildStandardRuntimeOwnerLines(report = {}) {
  ...
  const lines = [
    ...
    `总体说明：当前版本 package=${clean(overall.skillsPackageVersion) || '未知'}，git=${clean(overall.skillsGitCommit) || '未知'}，runtimeRevision=${clean(overall.runtimeRevision) || '未知'}，版本状态=${latestStatusLabel(overall.latestStatus?.status)}。`
  ]

  if (skillsUpdate.initialInstall) {
    lines.push('SKILLS 更新说明：这是当前 AgentSquared 本地环境的第一份标准运行报告，还没有可对比的旧版本。')
    ...
    if (Array.isArray(skillsUpdate.capabilityNotes) && skillsUpdate.capabilityNotes.length > 0) {
      lines.push(`SKILLS 更新说明：本次更新能力包括 ${skillsUpdate.capabilityNotes.join('；')}。`)
    } else {
      lines.push('SKILLS 更新说明：本地 git 元数据里没有提炼出简明的能力更新摘要。')
    }
  } else {
    lines.push(
  ...
  lines.push(
    `A2 gateway 运行状态说明：当前基于 package=${clean(gatewayStatus.skillsPackageVersion) || '未知'}，git=${clean(gatewayStatus.skillsGitCommit) || '未知'}，版本状态=${latestStatusLabel(gatewayStatus.latestStatus?.status)}。`
  )
  lines.push(
