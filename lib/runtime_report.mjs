...
export function buildStandardRuntimeOwnerLines(report = {}) {
  ...
  const lines = [
    'Standard AgentSquared runtime report:',
    `Overall: human=${clean(overall.humanName) || 'unknown'}${overall.humanId != null ? ` (id ${overall.humanId})` : ''}, agent=${clean(overall.agentId) || 'unknown'}, host=${clean(overall.hostRuntime) || 'none'}.`,
    `Identity: publicKey=${clean(overall.publicKey) || 'unknown'}, fingerprint=${clean(overall.publicKeyFingerprint) || 'unknown'}.`,
    `Official endpoints: relay=${clean(overall.relayApiBase) || 'unknown'}, skillsRepo=${clean(overall.skillsRepoUrl) || 'unknown'}.`,
    `Current version: package=${clean(overall.skillsPackageVersion) || 'unknown'}, git=${clean(overall.skillsGitCommit) || 'unknown'}, runtimeRevision=${clean(overall.runtimeRevision) || 'unknown'}.`
  ]

  if (skillsUpdate.initialInstall) {
    lines.push('Skills update: this is the first known local runtime report for the current AgentSquared setup.')
  } else if (skillsUpdate.changed) {
    lines.push(
      `Skills update: upgraded from ${clean(skillsUpdate.from?.gitCommit) || 'unknown'} (${clean(skillsUpdate.from?.packageVersion) || 'unknown'}) to ${clean(skillsUpdate.to?.gitCommit) || 'unknown'} (${clean(skillsUpdate.to?.packageVersion) || 'unknown'}).`
    )
    if (Array.isArray(skillsUpdate.capabilityNotes) && skillsUpdate.capabilityNotes.length > 0) {
      lines.push(`Updated capabilities: ${skillsUpdate.capabilityNotes.join(' | ')}`)
    } else {
      lines.push('Updated capabilities: local git metadata did not include a concise commit summary.')
    }
  } else {
    lines.push(
      `Skills update: no version change detected. Current package=${clean(skillsUpdate.to?.packageVersion) || 'unknown'}, git=${clean(skillsUpdate.to?.gitCommit) || 'unknown'}.`
    )
  }

  lines.push(
    `Gateway status: package=${clean(gatewayStatus.skillsPackageVersion) || 'unknown'}, git=${clean(gatewayStatus.skillsGitCommit) || 'unknown'}, latest=${clean(gatewayStatus.latestStatus?.status) || 'unknown'}, port=${clean(gatewayStatus.listeningPort) || 'unknown'}, peerId=${clean(gatewayStatus.peerId) || 'unknown'}.`
  )
  lines.push(
    `Gateway connectivity: relay=${gatewayStatus.relay?.ok ? 'ok' : 'failed'} (${clean(gatewayStatus.relay?.detail) || 'no detail'}), host=${gatewayStatus.hostRuntime?.ok ? 'ok' : 'failed'} (${clean(gatewayStatus.hostRuntime?.detail) || 'no detail'}).`
  )
  return lines
}
