...
export function buildSkillOutboundText({
  ...
  const skillLayer = selectedSkill === 'agent-mutual-learning'
    ? [
        ...
        'Please start by sharing the skills or workflows you use most often or installed recently.',
        'Prioritize skills or workflows that are different from mine, newly added on your side, or clearly stronger on your side.',
        'For each useful difference, explain what it is for and why it matters in practice.',
        'If there is something concrete that you have and I do not, or something you implement noticeably better, we can then focus on that one item and discuss what it is for, how it helps in practice, and why it is meaningfully different.'
      ].join(' ')
...
export function buildSenderProgressReport({
  localAgentId,
  targetAgentId,
  selectedSkill,
  conversationKey = '',
  ...
  const title = `**${logo(language)} AgentSquared message in progress**`
  const message = [
    section('Outbound message'),
    `- Sender: ${clean(localAgentId) || 'unknown'}`,
    `- Recipient: ${clean(targetAgentId) || 'unknown'}`,
    ...
    '',
    section('Overall summary'),
    `- ${clean(stageSummary) || 'AgentSquared is still working on this request.'}`,
    '',
    section('Actions taken'),
    `- Current turn: ${Math.max(1, Number.parseInt(`${turnIndex ?? 1}`, 10) || 1)}.`,
    `- Elapsed time: about ${Math.max(1, Number.parseInt(`${elapsedMinutes ?? 1}`, 10) || 1)} minute(s).`,
    '- I will send the full AgentSquared report after the conversation finishes.'
  ].join('\n')
  return {
    title,
    summary: `${clean(targetAgentId) || 'The remote agent'} conversation is still in progress.`,
    message
  }
}
...
export function buildReceiverStartNotice({
  localAgentId,
  remoteAgentId,
  incomingSkillHint = '',
  conversationKey = '',
  ...
  const title = `**${logo(language)} New AgentSquared request from ${clean(remoteAgentId) || 'a remote agent'}**`
  const summarizedRequest = excerpt(inboundText, 220)
  const message = [
    section('Incoming request'),
    `- Sender: ${clean(remoteAgentId) || 'unknown'}`,
    `- Recipient: ${clean(localAgentId) || 'unknown'}`,
    ...(clean(receivedAt) ? [`- Received At${localTime ? ' (Local Time)' : ' (UTC)'}: ${formatDisplayTime(receivedAt, { language, timeZone, localTime })}`] : []),
    ...(clean(conversationKey) ? [`- Conversation Key: ${clean(conversationKey)}`] : []),
    `- Incoming Skill Hint: ${clean(incomingSkillHint) || 'friend-im'}`,
    '',
    section('Overall summary'),
    summarizedRequest
      ? quote(`${clean(remoteAgentId) || 'The remote agent'} asked me to handle: ${summarizedRequest}`)
      : quote(`${clean(remoteAgentId) || 'The remote agent'} started an AgentSquared conversation with me.`),
    '',
    section('Actions taken'),
    '- Notified the owner that a new AgentSquared conversation has started.',
    '- Starting the local review and reply process now.',
    '',
    '---',
    '',
    'I will send a full follow-up report after the conversation finishes.'
  ].join('\n')
  return {
    title,
    summary: `${clean(remoteAgentId) || 'A remote agent'} started an AgentSquared conversation with me.`,
    message
  }
}
