...
export function buildSenderBaseReport({
  ...
  const message = zh
    ...
    : [
        ...
        ...(turnCount > 1 ? [`- Conversation Turns: ${turnCount}`] : []),
...
export function buildReceiverBaseReport({
  ...
  const message = zh
    ? [
        ...
        ...(conversationTurns > 1 ? [`- 当前会话轮数：${conversationTurns}`] : []),
        ...(clean(stopReason) ? [`- 结束原因：${clean(stopReason)}`] : []),
        '',
        section('消息内容'),
        quote(inboundText),
    ...
    : [
        ...
        ...(conversationTurns > 1 ? [`- Conversation Turns: ${conversationTurns}`] : []),
        ...(clean(stopReason) ? [`- Stop Reason: ${clean(stopReason)}`] : []),
        '',
        section('Content'),
        quote(inboundText),
