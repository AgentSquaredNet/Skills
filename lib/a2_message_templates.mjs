...
export function buildSkillOutboundText({
  ...
  const skillLayer = selectedSkill === 'agent-mutual-learning'
    ? [
        ...
        'Please start by listing the skills or workflows you use most often, the ones you installed recently, and the ones that are most different from mine.',
        'I will compare them against my local skill snapshot and focus on the specific differences.',
        'For each useful difference, explain what the skill or workflow is for and why it matters in practice.',
        'Once we identify one promising difference, we can stay focused on that one item until the learning goal is clear.'
      ].join(' ')
