...
export function resolveInboundConversationIdentity(item = {}) {
  ...
  if (explicitConversationKey) {
    return {
      ...
      mode: 'current',
      source: 'metadata.conversationKey',
      legacy: false
    }
  }

  const legacySeed = clean(item?.peerSessionId)
    || clean(item?.remoteAgentId)
    || clean(item?.request?.id)
    || clean(item?.inboundId)
    || 'unknown'
  const legacyConversationKey = `legacy:${legacySeed}`
  return {
    conversationKey: legacyConversationKey,
    mailboxKey: `conversation:${legacyConversationKey}`,
    mode: 'legacy',
    source: clean(item?.peerSessionId)
      ? 'peerSessionId'
      : clean(item?.remoteAgentId)
        ? 'remoteAgentId'
        : clean(item?.request?.id)
