...
export function createGatewayRuntimeState({
  ...
  function pruneExpiredTrustedSessions() {
    ...
    for (const [sessionId, session] of trustedSessions.entries()) {
      if (Date.parse(session.expiresAt) <= now) {
        ...
        const agentKey = session.remoteAgentId
        const known = trustedByAgent.get(agentKey)
        if (Array.isArray(known)) {
          const next = known.filter((value) => value !== sessionId)
          if (next.length > 0) {
            trustedByAgent.set(agentKey, next)
          } else {
            trustedByAgent.delete(agentKey)
          }
        }
      }
    }
  ...
  function rememberTrustedSession({
    ...
    const existing = Array.isArray(trustedByAgent.get(session.remoteAgentId))
      ? trustedByAgent.get(session.remoteAgentId)
      : []
    const next = existing.filter((value) => value !== session.peerSessionId)
    next.push(session.peerSessionId)
    trustedByAgent.set(session.remoteAgentId, next)
  ...
  function touchTrustedSession(peerSessionId) {
    ...
    const existing = trustedByAgent.get(session.remoteAgentId)
  ...
  function trustedSessionByAgent(remoteAgentId) {
    pruneExpiredTrustedSessions()
    const agentKey = `${remoteAgentId}`.trim()
    const sessionIds = trustedByAgent.get(agentKey)
    ...
    trustedByAgent.set(agentKey, next)
    const best = trustedSessions.get(next.at(-1))
