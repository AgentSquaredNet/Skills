...
import { buildStandardRuntimeOwnerLines, buildStandardRuntimeReport, currentSkillsMetadata } from './runtime_report.mjs'
...
export async function runGateway(argv) {
  ...
  const reportLanguage = `${args['report-language'] ?? process.env.AGENTSQUARED_REPORT_LANGUAGE ?? ''}`.trim()
  ...
  const detectedOpenClawAgent = `${detectedHostRuntime?.overviewStatus?.agents?.defaultId ?? detectedHostRuntime?.overviewStatus?.agents?.agents?.[0]?.id ?? ''}`.trim()
  ...
  function writeStateForNode(node) {
    ...
    writeGatewayState(gatewayStateFile, {
      ...
      skillsPackageVersion: currentSkills.packageVersion,
      skillsGitCommit: currentSkills.gitCommit,
      skillsRepoUrl: currentSkills.repoUrl,
      runtimeRevision,
    ...
    recoveryPromise = (async () => {
      ...
      try {
        ...
        console.log(JSON.stringify({
          ...
          runtimeState: runtimeState.snapshot(),
          standardReport,
          ownerFacingLines: buildStandardRuntimeOwnerLines(standardReport, { language: reportLanguage })
        }, null, 2))
        return nextNode
      } catch (error) {
