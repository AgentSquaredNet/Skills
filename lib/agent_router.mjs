import { buildReceiverStartNotice, inferOwnerFacingLanguage } from './a2_message_templates.mjs'
...
export function createAgentRouter({
  ...
  const scheduler = createMailboxScheduler({
    ...
    async handleItem(item, { mailboxKey, conversationKey }) {
      ...
      if (typeof notifyOwner === 'function' && startConversation.turnIndex === 1) {
        const inboundText = extractInboundText(item)
        const metadata = extractConversationMetadata(item)
        const ownerLanguage = inferOwnerFacingLanguage(inboundText)
        const startNotice = buildReceiverStartNotice({
          localAgentId: clean(metadata.to),
          remoteAgentId: item?.remoteAgentId,
          incomingSkillHint: clean(item?.suggestedSkill || item?.defaultSkill || selectedSkill),
          conversationKey: clean(startConversation.conversationKey) || clean(conversationKey),
