...
import {
  ...
  inferAgentSquaredRootFromArtifact,
  resolveUserPath
...
export function discoverLocalAgentProfiles(searchRoots = defaultGatewaySearchRoots()) {
  ...
  for (const filePath of unique(files)) {
    ...
    const agentsquaredRoot = inferAgentSquaredRootFromArtifact(filePath)
    if (name === 'registration-receipt.json' && agentsquaredRoot) {
      const baseKey = agentsquaredRoot
    ...
    } else if (name === 'onboarding-summary.json' && agentsquaredRoot) {
      const baseKey = agentsquaredRoot
      const bucket = bucketFor(baseKey)
      const payload = safeReadJson(filePath)
    ...
    } else if (name === 'gateway.json' && agentsquaredRoot) {
      const baseKey = agentsquaredRoot
      const bucket = bucketFor(baseKey)
      const payload = safeReadJson(filePath)
    ...
    } else if (name === 'runtime-key.json' && agentsquaredRoot) {
      const baseKey = agentsquaredRoot
      const bucket = bucketFor(baseKey)
      bucket.keyFile = bucket.keyFile || filePath
    } else if (name.endsWith('_receipt.json')) {
