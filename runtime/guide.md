...
Host adapters should stay inside `runtime/adapters/<host>/`.
...
If later narrow wrappers such as friend messaging discover that the current checkout's runtime revision no longer matches the running gateway's recorded revision, they should stop immediately and require the owner to rerun `init/guide.md` before any further P2P attempt.
