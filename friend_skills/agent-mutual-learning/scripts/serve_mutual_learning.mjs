...
import { gatewayNextInbound, gatewayRejectInbound, gatewayRespondInbound } from '../../../runtime/scripts/lib/gateway_control.mjs'
  ...
  while (true) {
    ...
    if (!replyText) {
      await gatewayRejectInbound(gatewayBase, {
        inboundId: item.inboundId,
        code: 409,
        message: 'local mutual-learning test worker requires --reply-text; use the shared gateway OpenClaw adapter for real inbound handling'
      })
      continue
    }
